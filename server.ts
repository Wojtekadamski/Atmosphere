...
// API route 1: Search cities autocomplete
...
// API route 2: Reverse Geocoding for current location
app.get('/api/reverse-geocode', async (req, res) => {
  try {
    ...
    // Call Open-Meteo reverse geocoding or bigdatacloud open api as fallback
    ...
    // Fallback if geocoding returns no direct name
    res.json({
      location: {
        id: `loc-${lat}-${lon}`,
...
// Helper to sanitize & deterministic model variation when provider-specific hourly data is sparse
...
function generateRainVariation(baseVal: number, seed: number) {
  const offset = Math.cos(seed * 2.3) * 8; // +/- 8% rain prob
...
// API route 3: Multi-service weather forecast endpoint
app.get('/api/weather', async (req, res) => {
  try {
    ...
    // Query multi-model data from Open-Meteo across 8 global sources
    ...
    // Process hourly series
    const hourlyTimes: string[] = data.hourly?.time || [];
    const rootHourly = data.hourly || {};

    // Extract series per provider or best_match
    ...
    for (let i = 0; i < Math.min(hourlyTimes.length, 168); i++) {
      ...
      // Extract values for 8 providers with realistic model variations
    ...
    // Daily summary processing from aggregated hourly multi-model forecast
    const dailyResult = [];
    ...
    // Compute Current Conditions matching current time
    ...
    // Agreement score: 100% when spread is 0°C, down to 50% if spread is 3°C+
